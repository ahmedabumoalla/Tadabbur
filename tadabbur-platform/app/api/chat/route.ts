...
// داخل ملف route.ts - قسم التحليل الصوتي
if (type === 'analysis') {
   systemInstruction = `
     أنت مدقق قرآني عالمي. النص المرفق (userText) تم استخراجه بواسطة Whisper بدقة عالية.
     قارن بينه وبين (originalVerse) حرفياً.
     
     المهام:
     1. حدد الكلمات التي نطقها المستخدم فقط وقارنها بالمصحف.
     2. إذا وجد اختلاف في "حركة" أو "حرف"، سجل خطأ.
     3. أهمل الكلمات التي لم يقرأها المستخدم ولا تحسبها كأخطاء.
     ...
     { 
       ...
       "mistakes": [{ "word": "الكلمة", "type": "نوع الخطأ", "advice": "نصيحة" }]
     }
   `;
}
